/* Archivo: pages/Careers/Create.tsx   Proposito: Pagina para crear registros de Careers.*/
...
const CreateCareer = () => {
  ...
  const handleCreateCareer = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const createdCareer = await careerService.createCareer(formData);
      if (createdCareer) {
        Swal.fire({
          title: 'Completado',
        ...
        Swal.fire({
          title: 'Error',
          text: 'Existe un problema al momento de crear la carrera',
          icon: 'error',
          timer: 3000,
        });
      }
